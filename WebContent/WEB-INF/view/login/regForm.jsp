...
/*	
  ...
  //핸드폰번호 체크 : 01[06789]\\d{3,4}\\d{4}가 맞는지
   ...
   $("#emailajax").on("change", function() {   // id 가져올때는 #을 붙여
         $("#emailajax").html("");   // #emailCheck는 span id
      if (emailRule.test($("#emailajax").val())) {
            var rCheck = r.rst;
            if(rCheck == "YYYY") {
               $("#emailajax").css("color", "green");
               $("#emailajax").html("사용가능한 Email 입니다.<br/><br/>");
            }else {
               $("#emailajax").css("color", "red");
               $("#emailajax").html("이미 사용중인 Email 입니다.<br/><br/>");
            }
      } else {
         $("#emailajax").css("color", "red");
         $("#emailajax").html("Email 형식에 맞지 않습니다.<br/><br/>");
      }
   });
   
   $("#pwajax").on("change", function() {
      $("pwajax").html("");
      if(passRule.test($(this).val())) {
         $("pwajax").css("color", "green");
         $("pwajax").html("사용가능한 비밀번호 입니다.<br/><br/>")
      }else {
   ...
   $("#phoneajax").on("change", function() {
      console.log($(this).val())
      $("phoneajax").html("");
      if(phoneRule.test($(this).val())) {
         $("#phoneajax").css("color", "green");
         $("#phoneajax").html("사용가능한 폰번호 입니다.<br/><br/>")
      }else {
         $("#phoneajax").css("color", "red");
         $("#phoneajax").html("폰번호 형식에 맞지 않습니다.<br/><br/>")   
      }
   });
...
<!-- 				<td><input type="password" name="password" class="size" placeholder=" 비밀번호입력" maxlength="20" required /> <small class="c" id="pass">8~20자 영문 대 소문자, 숫자, 특수문자를 사용하세요.</samll></td>  -->
				...
				<tr>
					<td class="n"> 주소</td>
					<td><input type="button" value="우편번호" id="zonecode" onclick="sample4_execDaumPostcode()" />
					<input type="text" name="addr" class="size" placeholder=" 주소입력" size="50" readonly maxlength="100자 이내" onclick="sample4_execDaumPostcode()" /><br />
					<input type="text" name="addr1" class="size" placeholder=" 상세주소입력" size="50" maxlength="100자 이내" /></td>
				</tr>
				<tr>
					<td class="n"> 핸드폰</td>
			 		<td><input type="text" name="contact" class="size" placeholder="ex)xxx-xxxx-xxxx" maxlength="11" required /><span id="phoneajax"></span></td> 
				</tr>
				<tr>
